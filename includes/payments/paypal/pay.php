            ...
            try{
                ...
                if (isset($_REQUEST['token']) && isset($_REQUEST['PayerID'])) {
                    ...
                    if (strtoupper($response['ACK']) == 'SUCCESS') {
                        ...
                        $response = sendNvpRequest( 'DoExpressCheckoutPayment', $data ,$config );
                        if ( 'SUCCESS' == strtoupper( $response['ACK'] ) || 'SUCCESSWITHWARNING' == strtoupper( $response['ACK'] ) ) {
                            // Get transaction info
                            $response = sendNvpRequest('GetTransactionDetails', array('TRANSACTIONID' => $response['PAYMENTINFO_0_TRANSACTIONID']),$config);
                            if ('SUCCESS' == strtoupper($response['ACK']) || 'SUCCESSWITHWARNING' == strtoupper($response['ACK'])) {
                                /*Success*/
