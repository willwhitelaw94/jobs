...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] == "installPayment") {
        installPayment();
    }
    if ($_GET['action'] == "uninstallPayment") {
        uninstallPayment();
    }
    if ($_GET['action'] == "installCountry") {
        installCountry();
    }
    if ($_GET['action'] == "uninstallCountry") {
        uninstallCountry();
    }
    ...
    if ($_GET['action'] == "deleteState") {
        deleteState();
    }
    if ($_GET['action'] == "deleteDistrict") {
        deleteDistrict();
    }
    if ($_GET['action'] == "deleteCity") {
        deleteCity();
    }

    if ($_GET['action'] == "deleteStaticPage") {
        deleteStaticPage();
    }
    if ($_GET['action'] == "deletefaq") {
        deletefaq();
    }
    if ($_GET['action'] == "delcoustomfield") {
        delcoustomfield();
    }
    if ($_GET['action'] == "approveitem") {
        approveitem();
    }
    if ($_GET['action'] == "approveResubmitItem") {
        approveResubmitItem();
    }
    ...
    if ($_GET['action'] == "deleteCurrency") {
        deleteCurrency();
    }
    if ($_GET['action'] == "deleteTimezone") {
        deleteTimezone();
    }
    if ($_GET['action'] == "deleteNoresult") {
        deleteNoresult();
    }
    if ($_GET['action'] == "deleteMembershipPlan") {
    ...
    if ($_GET['action'] == "deleteads") {
        deleteads();
    }
    if ($_GET['action'] == "deleteResubmitItem") {
        deleteResubmitItem();
    }
    ...
    if ($_GET['action'] == "edit_langTranslation") {
        edit_langTranslation();
    }
    if ($_GET['action'] == "langTranslation_PostTypes") {
        langTranslation_PostTypes();
    }
    if ($_GET['action'] == "edit_langTranslation_PostTypes") {
        edit_langTranslation_PostTypes();
    }
    if ($_GET['action'] == "langTranslation_SalaryTypes") {
        langTranslation_SalaryTypes();
    }
    if ($_GET['action'] == "edit_langTranslation_SalaryTypes") {
        edit_langTranslation_SalaryTypes();
    }
    if ($_GET['action'] == "langTranslation_FormFields") {
        langTranslation_FormFields();
    }
    if ($_GET['action'] == "edit_langTranslation_custom_fields") {
        edit_langTranslation_custom_fields();
    }
    if ($_GET['action'] == "CustomField_langTranslation_FormFields") {
        CustomField_langTranslation_FormFields();
    }

    if ($_GET['action'] == "addNewCat") {
        addNewCat();
    }
    ...
    if ($_GET['action'] == "addPostType") {
        addPostType();
    }
    if ($_GET['action'] == "editPostType") {
        editPostType();
    }
    if ($_GET['action'] == "delPostType") {
        delPostType();
    }

    if ($_GET['action'] == "addSalaryType") {
        addSalaryType();
    }
    if ($_GET['action'] == "editSalaryType") {
        editSalaryType();
    }
    if ($_GET['action'] == "delSalaryType") {
        delSalaryType();
    }
    ...
    if ($_GET['action'] == "editLanguageFile") {
        editLanguageFile();
    }

    if ($_GET['action'] == "saveBlog") {
        saveBlog();
    }
    ...
    if ($_GET['action'] == "addBlogCat") {
        addBlogCat();
    }
    if ($_GET['action'] == "editBlogCat") {
        editBlogCat();
    }
    if ($_GET['action'] == "delBlogCat") {
        delBlogCat();
    }

    if ($_GET['action'] == "deleteTestimonial") {
        deleteTestimonial();
    }

    if ($_GET['action'] == "deletereligion") {
        deleteReligion();
    }

    if ($_GET['action'] == "deleteUserLanguage") {
        deleteUserLanguage();
    }

    if ($_GET['action'] == "deleteCulturalBackground") {
        deleteCulturalBackground();
    }

    if ($_GET['action'] == "deleteAboutMe") {
        deleteAboutMe();
    }

    if ($_GET['action'] == "deletePreference") {
        deletePreference();
    }

    if ($_GET['action'] == "deleteInterest") {
        deleteInterest();
    }

    if ($_GET['action'] == "deleteCareExperience") {
        deleteCareExperience();
    }
    if ($_GET['action'] == "save_user_custom_fields") {
        save_user_custom_fields();
    }

    if ($_GET['action'] == "delete_user_custom_fields") {
        delete_user_custom_fields();
    }
    if ($_GET['action'] == "delete_user_custom_option") {
        delete_user_custom_option();
    }

    if ($_GET['action'] == "deleteDocuments") {
        deleteDocuments();
    }

    if ($_GET['action'] == "deleteRequirement") {
        deleteRequirement();
    }
}

if (isset($_POST['action'])) {
    if ($_POST['action'] == "quickad_update_maincat_position") {
        quickad_update_maincat_position();
    }
    if ($_POST['action'] == "quickad_update_subcat_position") {
        quickad_update_subcat_position();
    }
    if ($_POST['action'] == "quickad_update_post_type_position") {
        quickad_update_post_type_position();
    }
    if ($_POST['action'] == "quickad_update_salary_type_position") {
        quickad_update_salary_type_position();
    }
    if ($_POST['action'] == "quickad_update_blog_cat_position") {
        quickad_update_blog_cat_position();
    }
    if ($_POST['action'] == "quickad_update_custom_field_position") {
        quickad_update_custom_field_position();
    }
    if ($_POST['action'] == "quickad_update_custom_option_position") {
        quickad_update_custom_option_position();
    }
    if ($_POST['action'] == "quickad_update_user_custom_field_position") {
        quickad_update_user_custom_field_position();
    }
    if ($_POST['action'] == "quickad_update_user_custom_option_position") {
        quickad_update_user_custom_option_position();
    }
    ...
    if ($_POST['action'] == "delete_custom_fields") {
        delete_custom_fields();
    }
    if ($_POST['action'] == "delete_custom_option") {
        delete_custom_option();
    }
    if ($_POST['action'] == "save_custom_fields") {
        save_custom_fields();
    }
    if ($_POST['action'] == "save_user_custom_fields") {
        save_user_custom_fields();
    }
    if ($_POST['action'] == "delete_user_custom_fields") {
        delete_user_custom_fields();
    }
    if ($_POST['action'] == "delete_user_custom_option") {
        delete_user_custom_option();
    }
    ...
    if ($_POST['action'] == "getStateByCountryIDforCityAdd") {
        getStateByCountryIDforCityAdd();
    }
    if ($_POST['action'] == "getDistrictSelectedforCityAdd") {
        getDistrictSelectedforCityAdd();
    }
...
=======
    foreach ($fields as $custom) {
        ...
        $title = $custom['label'];
        $required = empty($custom['required']) ? 0 : $custom['required'];
>>>>>>> 0fadcd32ac7a7dea0ffccde6f9dffaaf3554dc3b
