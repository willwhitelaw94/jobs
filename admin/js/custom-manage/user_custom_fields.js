jQuery(function($) {
    ...
    $fields.sortable({
        ...
        update : function( event, ui ) {
            ...
            $.ajax({
                type : 'POST',
                url  : ajaxurl,
                data : { action: 'quickad_update_user_custom_field_position', position: data},
    ...
    $fields.on('click', '.quickad-js-delete', function(e) {
        ...
        var $item = $(this).closest('li'),
            ...
            data = { action: 'delete_user_custom_fields', id: id};
    ...
    $fields.on('click', '.quickad-option-delete', function(e) {
        ...
        var $item = $(this).closest('li'),
            ...
            data = { action: 'delete_user_custom_option', id: id, field_id: field_id};
    ...
    $('#ajax-send-user-custom-fields').on('click', function(e) {
        ...
        $.ajax({
            ...
            data      : { action: 'save_user_custom_fields', csrf_token : quickadL10n.csrf_token, fields: JSON.stringify(data), cf_per_service: $cf_per_service.val() },
            complete  : function() {
                ladda.stop();
     ...
     * @returns {*|jQuery}
     */
    function addField(type, id, label, required, customclass, custom_filter, translation) {
        ...
        $new_field.find('ul.quickad-items').sortable({
            ...
            update : function( event, ui ) {
                ...
                $.ajax({
                    type : 'POST',
                    url  : ajaxurl,
                    data : { action: 'quickad_update_user_custom_option_position',field_id: field_id, position: data},
                    success: function (response, textStatus, jqXHR) {
                        // Remove Ads item from DOM.
                        if(response != 0) {
        ...
        if (quickadL10n.custom_fields) {
            ...
            $.each(custom_fields, function (i, field) {
              //  var services = field.services.split(',');
                var $new_field = addField(field.type, field.id, field.label, field.required, field.custom_class, field.custom_filter,true);
