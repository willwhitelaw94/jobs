...
if( !empty($params['search']['value']) ) {
    ...
    $where .=" AND ( subadmin1_code = '".$_GET['code']."' )";
}

// getting total number records without any search
$sql = "SELECT * FROM `".$config['db']['pre']."subadmin2`";
$sqlTot .= $sql;
$sqlRec .= $sql;
...
if(isset($where) && $where != '') {
    ...
    $where .=" Where ( subadmin1_code = '".$_GET['code']."' )";
    $sqlTot .= $where;
    $sqlRec .= $where;
}
...
foreach ($queryRecords as $row) {
    ...
    $row5 = '<td class="text-center">
                <div class="btn-group">
                    <a href="#" data-url="panel/loc_district_edit.php?code='.$code.'" data-toggle="slidePanel" class="btn btn-xs btn-default"> <i class="ion-edit"></i> Edit</a>
                    <a href="javascript:void(0)" class="btn btn-xs btn-danger item-js-delete" data-ajax-action="deleteDistrict"> <i class="ion-close"></i></a>
                </div>
            </td>';
