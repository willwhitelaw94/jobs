...
var chat_id,post_id,user_id,full_name,user_image,user_status;
function agreementActionData(){
    var activechat=$('.active-chat').attr('id');
    var active_id=activechat.substring(activechat.indexOf('_')+1); 
    chat_id = $('#contact_'+active_id+'').data('chatid');
    post_id = $('#contact_'+active_id+'').data('postid');
    user_id = $('#contact_'+active_id+'').data('userid');
    full_name = $('#contact_'+active_id+'').data('fullname');
    user_image =$('#contact_'+active_id+'').data('userimage');
    user_status =$('#contact_'+active_id+'').data('userstatus');
}

function agreementAction(){

    if(( CHATID == null) && ($('.active-chat').length!=0)){
        agreementActionData();
    }
    if((CHATID == null) || ($('.active-chat').length!=0)){
        agreementActionData();
        ...
        post_id = POSTID;
        user_id = CHAT_USERID;
        full_name = CHAT_FULLNAME;
        user_image = CHAT_USERIMG;
        user_status=CHAT_USERSTATUS;   
    }
   // console.log(user_id);
    createAgreementBox(chat_id,user_id,post_id)  
